              ...
              {formData.images && formData.images.length > 0 && (
                <Stack spacing={0.75}>
                  {formData.images.map((url, idx) => (
                    <Stack key={idx} direction="row" alignItems="center" spacing={1.5} sx={{ px: 1.5, py: 1, borderRadius: '10px', bgcolor: (th) => alpha(th.palette.primary.main, 0.04), border: '1px solid var(--glass-border)' }}>
                      ...
                      <IconButton size="small" onClick={() => onRemoveImage(idx)} sx={{ color: 'error.main', borderRadius: '6px', '&:hover': { bgcolor: (th) => alpha(th.palette.error.main, 0.1) } }}><CloseRoundedIcon sx={{ fontSize: 13 }} /></IconButton>
                    </Stack>
                  ))}
                </Stack>
              )}
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3, lg: 4 }, maxWidth: 1400, mx: 'auto' }}>
      ...
      <FormDrawer
        ...
        onImageUrlChange={setImageUrl} onAddImage={addImage} onAddImages={addImages} onRemoveImage={removeImage}
