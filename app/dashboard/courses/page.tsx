...
export default function CoursesPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await courseService.adminGetAll({
        limit: 100,
        include_hidden: true,
  ...
  const handleDelete = async (id: number, hard = false) => {
    ...
    try { await courseService.remove(id, hard); await fetchData(); }
          ...
          <Card className="fade-in-up" sx={{ animationDelay: '150ms' }}>
            <CardContent sx={{ py: 10, textAlign: 'center' }}>
              <Box sx={{
                ...
                <MenuBookRoundedIcon sx={{ fontSize: 36, color: '#fff' }} />
              </Box>
                ...
                >
                  ...
                  {item.images && item.images.length > 0 ? (
                    ...
                    <Box sx={{
                      ...
                      <MenuBookRoundedIcon sx={{ fontSize: 40, color: 'text.disabled', opacity: 0.5 }} />
                    </Box>
                  )}
                  ...
                  <CardContent sx={{ flex: 1, pb: 1.5 }}>
                      ...
                      <Stack spacing={0.5} sx={{ mt: 1 }}>
                        {platform && (
                          <Stack direction="row" alignItems="center" spacing={0.5}>
                            ...
                            {instructor && <Typography variant="caption" color="text.secondary"> · {instructor}</Typography>}
                          </Stack>
                        )}
                        {duration && (
                          <Stack direction="row" alignItems="center" spacing={0.5}>
                    ...
                    <Stack direction="row" spacing={2} sx={{ mt: 1.5 }}>
                      {url && (
                        <Typography variant="caption" component="a" href={url} target="_blank" rel="noopener noreferrer" sx={{
                          display: 'inline-flex', alignItems: 'center', gap: 0.5,
                          textDecoration: 'none', color: 'text.secondary', fontWeight: 600,
                          transition: 'color 0.2s', '&:hover': { color: 'primary.main' },
                        }}>
        ...
        <form onSubmit={handleSubmit}>
          <DialogContent dividers sx={{ borderColor: 'var(--glass-border)' }}>
            <Stack spacing={3}>
              ...
              <Stack direction="row" spacing={1} alignItems="flex-end">
                <TextField label={t('form.metaKey')} value={metaKey} onChange={(e) => setMetaKey(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addMetadata(); } }} size="small" sx={{ width: 130 }} placeholder="platform" />
                <TextField label={t('form.metaValue')} value={metaValue} onChange={(e) => setMetaValue(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addMetadata(); } }} size="small" sx={{ flex: 1 }} placeholder='Udemy or ["React","Node.js"]' />
