...
type FormData = ExperienceCreate & { id?: number };
      ...
      <Card sx={{
        ...
        {item.images && item.images.length > 0 ? (
          ...
          <Box sx={{
            ...
            <WorkRoundedIcon sx={{ fontSize: 52, opacity: 0.25, color: 'primary.main' }} />
          </Box>
        )}
                      ...
                      <TableCell sx={{ borderBottom: '1px solid var(--glass-border)' }}>
                        {company ? (
                          <Stack direction="row" alignItems="center" spacing={0.5}>
                            ...
                            <Typography variant="caption" fontWeight={600} color="text.secondary">{company}</Typography>
                          </Stack>
        ...
        <Accordion defaultExpanded disableGutters elevation={0} sx={accordionSx}>
          <AccordionSummary expandIcon={<KeyboardArrowDownRoundedIcon />} sx={accordionSummarySx}>
            <Stack direction="row" alignItems="center" spacing={1}>
              <WorkRoundedIcon sx={{ fontSize: 17, color: 'primary.main' }} />
        ...
        <Accordion disableGutters elevation={0} sx={accordionSx}>
          ...
          <AccordionDetails sx={{ pt: 0, pb: 2 }}>
            <Stack spacing={2}>
              ...
              <UrlField label={t('form.companyUrl')} value={formData.company_url || ''} onChange={(v) => onFieldChange('company_url', v)} />
              <UrlField label={t('form.companyLogoUrl')} value={formData.company_logo_url || ''} onChange={(v) => onFieldChange('company_logo_url', v)} />
            ...
            <Stack spacing={1.5}>
              ...
              <Stack direction="row" spacing={1} alignItems="flex-end">
                <TextField label={t('form.metaKey')} value={metaKey}
                  ...
                  size="small" sx={{ width: 120 }} placeholder="company" />
                <TextField label={t('form.metaValue')} value={metaValue}
                  onChange={(e) => onMetaValueChange(e.target.value)}
                  onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); onAddMetadata(); } }}
                  size="small" sx={{ flex: 1 }} placeholder='Google or ["React","Node.js"]' />
...
export default function ExperiencePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else await experienceService.create({ ...formData } as ExperienceCreate);
  ...
  const handleDelete = async (id: number) => {
    try { await experienceService.remove(id, false); await fetchData(); }
    catch (err: unknown) { alert(err instanceof Error ? err.message : t('errors.deleteFailed')); }
  };
  const handleHardDelete = async (id: number) => {
    try { await experienceService.remove(id, true); await fetchData(); }
    catch (err: unknown) { alert(err instanceof Error ? err.message : t('errors.deleteFailed')); }
  };
  const handleRestore = async (id: number) => {
    try { await experienceService.restore(id); await fetchData(); }
              ...
              <CardContent sx={{ py: 12, textAlign: 'center' }}>
                <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}>
                  <Box sx={{
                    ...
                    <WorkRoundedIcon sx={{ fontSize: 40, color: '#fff' }} />
                  </Box>
                </motion.div>
