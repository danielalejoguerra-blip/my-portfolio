...
export default function ProjectsPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await projectService.adminGetAll({
        limit: 100,
        include_hidden: true,
  ...
  const handleDelete = async (id: number, hard = false) => {
    ...
    try { await projectService.remove(id, hard); await fetchData(); }
          ...
          <Card className="fade-in-up" sx={{ animationDelay: '150ms' }}>
            <CardContent sx={{ py: 10, textAlign: 'center' }}>
              <Box sx={{
                ...
                <FolderRoundedIcon sx={{ fontSize: 36, color: '#fff' }} />
              </Box>
                ...
                >
                  ...
                  {item.images && item.images.length > 0 ? (
                    ...
                    <Box sx={{
                      ...
                      <FolderRoundedIcon sx={{ fontSize: 48, color: 'text.disabled', opacity: 0.5 }} />
                    </Box>
                  )}
                  ...
                  <CardContent sx={{ flex: 1, pb: 1.5 }}>
                    ...
                    <Stack direction="row" spacing={2} sx={{ mt: 1.5 }}>
                      {typeof meta.github === 'string' && meta.github && (
                        <Typography variant="caption" component="a" href={meta.github} target="_blank" rel="noopener noreferrer" sx={{
                      ...
                      {typeof meta.live === 'string' && meta.live && (
                        <Typography variant="caption" component="a" href={meta.live} target="_blank" rel="noopener noreferrer" sx={{
                          display: 'inline-flex', alignItems: 'center', gap: 0.5,
                          textDecoration: 'none', color: 'text.secondary', fontWeight: 600,
                          transition: 'color 0.2s', '&:hover': { color: 'primary.main' },
                        }}>
                          <LanguageRoundedIcon sx={{ fontSize: 14 }} />Live
                        </Typography>
                      )}
                      {item.images && item.images.length > 0 && (
            ...
            <Stack spacing={3}>
              ...
              <Stack direction="row" spacing={1} alignItems="flex-end">
                ...
                <TextField
                  ...
                  size="small"
                  sx={{ flex: 1 }}
                  placeholder='https://... or ["React","Node.js"]'
                />
